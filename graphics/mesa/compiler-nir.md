...
- `nir_intrinsic_info`
  ...
  - `flags` are
    - `NIR_INTRINSIC_CAN_ELIMINATE` means the intrin has no side effect and
...
- `enum glsl_sampler_dim sampler_dim`
  - note that a complete glsl type also includes `shadow`, `array`, and `type`
  - `GLSL_SAMPLER_DIM_1D`
    - glsl: `gsampler1D`
  - `GLSL_SAMPLER_DIM_2D`
    - glsl: `gsampler2D`
  ...
  - `GLSL_SAMPLER_DIM_EXTERNAL`
    - glsl: `samplerExternalOES`
  - `GLSL_SAMPLER_DIM_MS`
    - glsl: `gsampler2DMS`
  - `GLSL_SAMPLER_DIM_SUBPASS`
    - glsl: `gsubpassInput`
  - `GLSL_SAMPLER_DIM_SUBPASS_MS`
    - glsl: `gsubpassInputMS`
- `nir_alu_type dest_type`
  - this is normally `nir_type_{float,int,uint}32`, but
    `nir_opt_16bit_tex_image` might lower it to `nir_type_{float,int,uint}16`
- `nir_texop op`
...
- `nir_def def`
  - `num_components` is commonly 4 and `bit_size` is commonly 32
- `nir_tex_src *src` and `unsigned num_srcs`
  - `src[0]` is often `nir_tex_src_texture_deref` which is often lowered to
    `nir_tex_src_texture_handle`
  - `src[1]` is often `nir_tex_src_sampler_deref`, which is often lowered to
    `nir_tex_src_sampler_handle`, if the op requires a sampler
  - `src[2]` is often `nir_tex_src_coord`, if the op requires a coord
  - etc.
...
- `unsigned coord_components` is the component count of src coord
  - it depends on sampler dim and array
- `bool is_array`
- `bool is_shadow`
- `bool is_new_style_shadow` indicates the newer glsl `texture` rather than
  the legacy glsl `shadow2D()`
- `bool is_sparse` indicates the glsl `sparseTextureARB()`, etc. from
  `GL_ARB_sparse_texture2`
- `unsigned component : 2` indicates the component if op is `nir_texop_tg4`
- `unsigned array_is_lowered_cube : 1` is r600-specific
- `unsigned is_gather_implicit_lod : 1` is for
  `GL_AMD_texture_gather_bias_lod`
- `int8_t tg4_offsets[4][2]` is for `textureGatherOffset`
- `bool texture_non_uniform` and `bool sampler_non_uniform` are mainly for
  non-uniform indexing of descriptors, where each invocation might refer to a
  different texture/sampler
  - e.g., `layout(set = 0, binding = 0) uniform sampler2D texs[64];` and
    `texture(texs[nonuniformEXT(idx)], coords)`
- `unsigned texture_index` and `unsigned sampler_index` are GL-specific
  - while vulkan lowers `nir_tex_src_texture_deref` to
    `nir_tex_src_texture_handle`, gl uses `nir_lower_samplers` to lower
    `nir_tex_src_texture_deref` to `texture_index` when
    `PIPE_CAP_NIR_SAMPLERS_AS_DEREF` is false
  - e.g., `layout(location = 2, binding = 3) uniform sampler2D s;`
    - it is the same as `glUniformi(2, 3)`
    - it binds the sampler to unit 3
    - `texture_index` is 3
...
- `process_intrinsic` is called on all intrisics
  - it adds `ACCESS_NON_WRITEABLE`, `ACCESS_NON_READABLE`, and
    `ACCESS_CAN_REORDER` to `nir_intrinsic_access`, if the access can be
