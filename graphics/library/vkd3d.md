...
- win64 with gcc
  ...
  - `meson setup out-win64 --cross-file build-win64.txt -Dprefix=$PWD/out-win64/install -Dbuildtype=debug`
  - win32 is similar
- win64 with clang
  - <https://github.com/mstorsjo/llvm-mingw/releases>
    - pick `llvm-mingw-<date>-ucrt-ubuntu-22.04-x86_64.tar.xz`
      - `ucrt` vs `msvcrt` are target C runtimes
      - `ubuntu-22.04` is cross-compiler for ubuntu
  ...
  - simply use `build-win64.txt` as the cross file
    - llvm-mingw bundles everything, including `gcc` wrappers that invokes
      `clang`
- aarch64 with clang
  - `sed -i 's/x86_64/aarch64/' build-win64.txt`
  - this is not too useful unless the game is aarch64
- arm64ec with clang
