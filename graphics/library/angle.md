...
- `out/android/angle_end2end_tests` script internals
  - wrappers
    - it is generated by `build/android/gyp/generate_android_wrapper.py`
    - it wraps `out/android/bin/run_angle_end2end_tests` which wraps
      `testing/test_env.py src/tests/angle_android_test_runner.py gtest --suite angle_end2end_tests ..`
    - `src/tests/angle_android_test_runner.py` uses
      - `build/android/test_runner.py` by default
        - this is much more complex and uses `third_party/catapult/devil`
      - `src/tests/py_utils/android_helper.py` if `--angle-test-runner` or
        `angle_trace_tests`
  - `build/android/test_runner.py`
    - `RunTestsCommand -> RunTestsInPlatformMode`
    - `environment_factory.CreateEnvironment` creates `LocalDeviceEnvironment`
    - `test_instance_factory.CreateTestInstance` creates `GtestTestInstance`
      - it inits `apk_path` to `angle_end2end_tests-debug.apk`
      - `device_dependencies.GetDataDependencies` parses
        `out/android-arm64/gen.runtime/src/tests/angle_end2end_tests__test_runner_script.runtime_deps`
        for test data, etc.
    - `test_run_factory.CreateTestRun` creates `LocalDeviceGtestRun`
    - `with test_run` calls `LocalDeviceTestRun.SetUp`
      - `install_apk` installs `angle_end2end_tests-debug.apk`
    - `LocalDeviceTestRun.RunTests`
      - `LocalDeviceGtestRun._GetTests` returns available tests
        - `list_tests` calls `_ApkDelegate::Run` and parses logcat
      - `LocalDeviceGtestRun._RunTest` runs a single test
        - it calls `_ApkDelegate::Run` and parses logcat
