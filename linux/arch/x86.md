...
- current task
  ...
  - it returns `pcpu_hot.current_task` per-cpu variable
- task switch
  ...
  - `__switch_to`
    - sets `pcpu_hot.current_task` to `next_p`
    - sets `pcpu_hot.top_of_stack` to `task_top_of_stack(next_p)`
    - others
...
- IDT entries from `FIRST_EXTERNAL_VECTOR` to `NR_VECTORS` are initialized
  ...
  - `asm_common_interrupt`, the handler, is defined by `idtentry_irq`
    - it saves `pt_regs` and switches to the kernel stack
