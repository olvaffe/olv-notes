...
- when one clones a git repo with submodules, one should
  - `git submodule init` registers submodules
    - its add `[submodule]` section to `.git/config`
    - `.gitmodules` serves as a template
  - `git submodule update` clones all registered submodules and checks out the
    recorded commit hashes
    - `--init` registers all submodules automatically
    - `--recursive` updates nested submodules recursively
